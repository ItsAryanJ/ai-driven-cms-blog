...
function TagContent() {
  ...
  useEffect(() => {
    const fetchPosts = async () => {
      setLoading(true);
      try {
        const data = await postsService.getPublished({ page, page_size: 9, tag: slug });
        setPostsData(data);
      } catch {
        setPostsData(null);
      } finally {
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-12">
      <div className="mb-8">
        <p className="text-sm text-violet-400 font-medium mb-2">Tag</p>
        <h1 className="text-3xl font-bold text-white">
          # {slug.replace(/-/g, " ")}
        </h1>
        <p className="text-gray-400 mt-2">{postsData?.total || 0} posts with this tag</p>
      </div>
      ...
      {postsData && postsData.items.length > 0 ? (
        <>
          ...
          <Suspense fallback={null}>
            <Pagination currentPage={postsData.page} totalPages={postsData.total_pages} basePath={`/tag/${slug}`} />
          </Suspense>
        </>
      ) : (
